...
If the client has authenticated via [`sasl`], the client MUST include an
`Authorization` header field. If the `PLAIN` SASL mechanism was used, the
client MUST use the HTTP `Basic` authorization scheme. If the `OAUTHBEARER`
SASL mechanism was used, the client MUST use the HTTP `Bearer` authorization
scheme.
